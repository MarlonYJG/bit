...
export class UiUI {
  ...
  async renderSsr(rootExtension: string, { assets, browser }: SsrContent = {}) {
    ...
    // TODO - verify href includes protocol
    const serverUrl = browser?.location.href || 'http://localhost:3000';
    ...
    try {
      // consider rendering `<Html/>` to static markup
      const content = ReactDOMServer.renderToString(
        <Html title="bit dev ssred!" assets={{ ...assets, state, ...realtimeAssets }}>
          {app}
        </Html>
      );
